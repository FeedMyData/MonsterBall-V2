...
/////// Vectors:
        ...
        Pass {
            ...
            uniform float _node_1632;
            uniform float _Neg_copy;
            uniform sampler2D _Yeux; uniform float4 _Yeux_ST;
            struct VertexInput {
                float4 vertex : POSITION;
                float3 normal : NORMAL;
                float2 texcoord0 : TEXCOORD0;
...
/////// Diffuse:
                ...
                float4 _Yeux_var = tex2D(_Yeux,TRANSFORM_TEX(i.uv0, _Yeux));
                float3 diffuse = (directDiffuse + indirectDiffuse) * _Yeux_var.rgb;
/// Final Color:
...
/////// Diffuse:
                ...
                float3 directDiffuse = max( 0.0, NdotL) * attenColor;
                float4 _Yeux_var = tex2D(_Yeux,TRANSFORM_TEX(i.uv0, _Yeux));
                float3 diffuse = directDiffuse * _Yeux_var.rgb;
/// Final Color:
                float3 finalColor = diffuse;
